...
#define ESP32CAM_SUBSCRIBE_TOPIC   "cam1_controls" 
...
void logimage(){
  ...
  if(fb != NULL && fb->format == PIXFORMAT_JPEG && fb->len < bufferSize){
    ...
    if(imgurImage > 0){
      snprintf(msg, BUFFER_LEN, "{\"camera\":\"Main\", \"link\":\"%s\"}", imgurUploader.getURL());
...
void setup() {
  ...
  //Disabling Brownout error: Voltage error disabled
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0);
  
  /*
  //Watch Dog Timer Code
  ...
  config.pixel_format = PIXFORMAT_JPEG;
  if(psramFound()){
    config.frame_size = FRAMESIZE_VGA; //600x480
...
void loop() {
  ...
  if (minute <= _min && !executed){
    ...
    //Posting to DropBox
    SPIFFS.begin();
    File logs = SPIFFS.open("/logs.csv", "a");
    logs.println(minute);
    logs.close();
    SPIFFS.end();

    myDropbox.begin("SZutwliEzpcAAAAAAAAAAbQ5pF1DnQf8WewXQVHsZlO436UY-0n4pVfysUePtRir");
    if(myDropbox.fileUpload("/logs.txt", "/logs.txt", 1)){
      Serial.println("logs.csv updated!");
    } else {
      Serial.println("No new logs");
    }
