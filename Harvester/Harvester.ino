...
#define ESP32CAM_PUBLISH_TOPIC   "esp32/cam_3"
#define ESP32CAM_PUBLISH_TOPIC2   "image_log/cam" 
#define ESP32CAM_SUBSCRIBE_TOPIC   "cam4_controls" 
...
void Harvested(float Reading){
  snprintf(msg, BUFFER_LEN, "{\"name\":\"harvest\", \"data\":%f}", Reading);
  client.publish(ESP32CAM_PUBLISH_TOPIC3, msg);
  Serial.println("Logged Reading!");
  Serial.setTimeout(10000);
}

void SeedsSown(float Reading){
  snprintf(msg, BUFFER_LEN, "{\"name\":\"sown\", \"data\":%f}", Reading);
  client.publish(ESP32CAM_PUBLISH_TOPIC3, msg);
  Serial.println("Logged Reading!");
  Serial.setTimeout(10000);
}
...
void logimage(){
  ...
  if(fb != NULL && fb->format == PIXFORMAT_JPEG && fb->len < bufferSize){
    ...
    if(imgurImage > 0){
      snprintf(msg, BUFFER_LEN, "{\"camera\":\"Harvester\", \"link\":\"%s\"}", imgurUploader.getURL());
